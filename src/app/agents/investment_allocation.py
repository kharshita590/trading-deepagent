...
class InvestmentAllocationAgent:
    def __init__(self, llm_model="gpt-4", indian_stock_tickers_file=None):
    ...
    async def evaluate_market_conditions(self, state: InvestmentAllocationState) -> InvestmentAllocationState:
        ...
        try:
            ...
            volatility = hist["returns"].rolling(21).std().iloc[-1] * np.sqrt(252)  # annualized vol
            ...
            for sector, etf in sector_etfs.items():
                data = yf.download(etf, period="1mo", interval="1d")
                if not data.empty:
                    ret = (data["Close"][-1] / data["Close"][0]) - 1
            ...
            sector_rotation = True if max(sector_returns.values()) - min(sector_returns.values()) > 0.05 else False

            sector_hist = pd.DataFrame()
            for sector, etf in sector_etfs.items():
                data = yf.download(etf, period="3mo", interval="1d")["Close"].pct_change()
                sector_hist[sector] = data
            corr_matrix = sector_hist.corr()
            avg_corr = corr_matrix.mean().mean()
            correlation_levels = "low" if avg_corr < 0.3 else "moderate" if avg_corr < 0.6 else "high"

            avg_volume = hist["Volume"].mean()
            liquidity_conditions = "good" if avg_volume > 50_000_000 else "poor"
            ...
            if market_conditions["market_volatility"] == "high":
                state["allocation_factors"]["volatility_adjustment"] = -0.2
            elif market_conditions["market_volatility"] == "low":
            ...
            state["market_conditions"] = {"error": str(e)}
