...
from app.core.yfinance_utils import fetch_financial_bundle, fetch_history

from ..config.settings import logger, SECTOR_ETFS
...
class FinancialDataProvider:
    ...
    async def get_financial_metrics(ticker: str) -> Dict:
        try:
            bundle = await fetch_financial_bundle(ticker)
            info = bundle.get("info", {})
            financials = bundle.get("financials")
            balance_sheet = bundle.get("balance_sheet")
            ...
            metrics = {
                ...
                "has_balance_sheet": balance_sheet is not None and not balance_sheet.empty,
                "has_cashflow": cashflow is not None and not cashflow.empty,
            ...
            logger.error("Error fetching financial data for %s: %s", ticker, e)
            return {"error": str(e), "ticker": ticker}

    @staticmethod
    async def get_peer_comparison(ticker: str, sector: str) -> Dict:
        try:
            sector_etf = SECTOR_ETFS.get(sector, "SPY")
            etf_history = await fetch_history(sector_etf, period="1y")
            etf_info_bundle = await fetch_financial_bundle(sector_etf)
            etf_info = etf_info_bundle.get("info", {})
            return {
            ...
            logger.error("Error fetching peer comparison for %s: %s", ticker, e)
            return {"error": str(e)}
    ...
    def calculate_financial_scores(metrics: Dict) -> Dict:
        ...
        try:
            ...
            logger.error("Error calculating financial scores: %s", e)
            scores = {"error": str(e)}
        return scores
...
class FinancialDataFetchAgent:
    ...
    async def execute(self, state: FundamentalAgentState) -> FundamentalAgentState:
        ...
        for rec in recommendations:
            ...
            peer_data = await self.data_provider.get_peer_comparison(ticker, sector)
